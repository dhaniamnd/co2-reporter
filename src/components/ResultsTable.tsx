        ...
        <tbody>
          {rows.map((r,i) => (
            <tr key={i} className="odd:bg-[var(--brand-102)] even:bg-[var(--brand-103)]">
              ...
              <td className="px-3 py-2 text-right">{r.input.Clinker_t.toLocaleString()}</td>
              <td className="px-3 py-2 text-right">{r.input.KilnFuel_GJ.toLocaleString()}</td>
              <td className="px-3 py-2 text-right">{r.input.Electricity_MWh.toLocaleString()}</td>
              <td className="px-3 py-2 text-right">{r.Process_tCO2.toFixed(2)}</td>
              <td className="px-3 py-2 text-right">{r.Fuel_tCO2.toFixed(2)}</td>
              <td className="px-3 py-2 text-right">{r.Electric_tCO2.toFixed(2)}</td>
              <td className="px-3 py-2 text-right font-semibold">{r.Total_tCO2.toFixed(2)}</td>
            </tr>
          ))}
        </tbody>
        <tfoot>
